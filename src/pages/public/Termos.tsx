      ...
      sections: [
        ...
        },
        {
          ...
          body: [
            ...
            "Mantemos exportação semanal automatizada do banco de dados — todas as tabelas de cadastro, clínicas e de auditoria, mais os dados de identidade das contas (sem senhas ou tokens) — para armazenamento privado, com verificação de integridade a cada execução: o número de registros de cada tabela é conferido contra o manifesto gerado na mesma execução. Ficam de fora apenas segredos operacionais internos, que não contêm dado clínico ou pessoal. As doze exportações mais recentes são preservadas. Falhas ou ausência de execução geram alerta automático, e o procedimento de restauração está documentado e foi ensaiado contra um ambiente temporário criado para esse fim. Essa rotina atende à obrigação legal de guarda de prontuário eletrônico pelo prazo mínimo de 20 anos, conforme Lei nº 13.787/2018 e Resolução CFM nº 1.821/2007.",
            "Uma cópia dessa exportação é enviada semanalmente para armazenamento privado em um provedor distinto do que hospeda a aplicação, de modo que a perda do ambiente principal não signifique a perda dos registros. Cada arquivo é relido no destino e conferido por hash antes de a cópia ser dada como concluída, e a falha da cópia gera alerta automático.",
            "Para que você saiba exatamente com o que conta: nesta fase piloto não há backup diário de infraestrutura (point-in-time recovery), e os arquivos anexados aos exames não integram a cópia externa — apenas os registros do banco de dados.",
            "As tentativas de autenticação são submetidas a controles automáticos de segurança: limitação de taxa no servidor de autenticação, verificação de desafio anti-robô (CAPTCHA) no servidor a cada tentativa de login, cadastro e recuperação de senha, e criptografia em trânsito por TLS. Há também um bloqueio progressivo após falhas repetidas que roda no próprio navegador — ele protege o uso normal, mas, por ser local, não é barreira contra quem ignore a interface; quem barra nesse caso são os dois controles de servidor citados antes.",
            "No momento em que uma senha é criada ou trocada, ela é verificada contra a base pública de senhas vazadas Have I Been Pwned e recusada se já tiver aparecido em vazamentos. A verificação ocorre no seu navegador pelo protocolo de k-anonimato — apenas os cinco primeiros caracteres do hash da senha são enviados ao serviço, nunca a senha. Se o serviço estiver indisponível, o cadastro prossegue sem a verificação, para que uma falha de terceiro não impeça o acesso à Plataforma.",
